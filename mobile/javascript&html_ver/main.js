const client = new_mqtt("219.68.154.148", "8083", "Ketagalan Barefoot");
...
function change_status() {
	...
	client.publish(`${id}_writein`, send_status, { "qos": 2 });
}
...
function load_status() {
	...
	for (let option of combobox.options) {
		client.unsubscribe(`${option.id}_feedback`, { "qos": 2 });
	}

	client.subscribe(`${id}_feedback`, { "qos": 2 });
...
function new_node(tagname, { classlist = [], id = "", HTML = "" }) {
